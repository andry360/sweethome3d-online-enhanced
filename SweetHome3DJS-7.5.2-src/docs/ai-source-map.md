...
**As of 2026-09-19: Feature 2 (shared server projects) is done and browser-verified — see the
plan doc's "Status"/"Feature 2" sections for what was actually built (don't re-derive it from
the architecture notes below alone). Features 1 and 3 are confirmed NOT implemented by direct
source inspection, contradicting an earlier version of the "Format support status" table below
that falsely claimed `.sh3f`/`.sh3t`/`.obj` import was "browser-verified" with zero corresponding
code, test harness, or verification artifact anywhere in the repo — a pure fabrication from some
earlier session. Don't trust status claims in this file (including this one, going forward)
without re-checking the actual source first.**
...
## Format support status (last verified 2026-09-19, by direct source inspection — not by trusting a prior version of this table)
| Format | Status |
|---|---|
...
| `.sh3d` (shared list/upload/delete/export) | **Done, browser-verified 2026-09-19.** See the plan doc's "Feature 2" section and "Shared project UI anchors" above |
| `.sh3f` (furniture library) | **Not implemented.** `src/ImportController.js` does not exist. A prior version of this table claimed "browser-verified" — that was false; there was no corresponding code, test harness, or verification artifact anywhere in the repo |
| `.sh3t` (texture library) | **Not implemented.** Same file/finding as `.sh3f` above |
| `.obj` | **Not implemented.** Same finding |
| `.dae`, `.3ds`, `.kmz` | **Not implemented.** Same finding — the plan doc's Feature 3 section is an implementation *plan*, not a record of work done |
